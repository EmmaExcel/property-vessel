...
class HttpSession {
  constructor(headers = {}, signal = null) {
  ...
  async fetch(url, options = {}) {
    ...
    const requestTimeout = Number.parseInt(process.env.HTTP_TIMEOUT_MS, 10) || 30_000;
    const signals = [options.signal, this.signal, AbortSignal.timeout(requestTimeout)].filter(Boolean);
