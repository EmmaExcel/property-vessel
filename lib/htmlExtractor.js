...
function isLikelyPropertyImage(value) {
  ...
  if (/(?:^|[\/_-])(?:logos?|favicons?|sprites?|avatars?|icons?|markers?|loading|placeholders?|spacers?|blank)(?:[\/_\-.]|$)/i.test(url)) return false;
  if (/broadbandavailability|broadband(?:uk)?[-_]?badge/i.test(url)) return false;
  return true;
}
...
function srcsetUrls(value) {
  ...
  // Standard srcset values are comma-separated URL/width pairs. A few estate
  // agent themes instead use `>768:https://... , >1200:https://...`. Extract
  // each embedded URL so the breakpoint marker never becomes part of it.
  const embedded = [...text.matchAll(/https?:\/\/[^\s,]+/gi)].map((match) => match[0]);
  if (embedded.length) return embedded;
  return text.split(',').map((candidate) => candidate.trim().split(/\s+/)[0]).filter(Boolean);
}
