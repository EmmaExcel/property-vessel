...
const STATUS_ONLY_TITLE = /^(?:sold(?:\s+stc)?|under\s+offer|new\s+build|available|for\s+sale|to\s+let|let\s+agreed|apartment\s+living)$/i;
const PAGE_TITLE_NOISE = /(?:properties?\s+archive|search\s+results?|page\s+\d+\s+of\s+\d+|\.st\d+\s*\{|fill\s*:\s*#|<svg|<style|@keyframes)/i;
...
function urlArray(value) {
  ...
  function visit(item, depth = 0) {
    if (item === null || item === undefined || depth > 6) return;
    ...
    if (typeof item === 'object') {
      const direct = item.url || item.src || item.href || item.contentUrl || item.srcUrl;
      if (direct) visit(direct, depth + 1);
      else Object.values(item).forEach((entry) => visit(entry, depth + 1));
    }
  }
  visit(value);
...
function cleanPlatformText(value, field) {
  ...
  if (field === 'address' && /^\s*\d{3}\s*(?:\r?\n|\s{3,})/.test(text)) {
    text = text.replace(/^\s*\d{3}\s*(?:\r?\n|\s{3,})+/, '');
  }
  text = field === 'description'
    ? text.replace(/[ \t]+/g, ' ').replace(/\n\s*\n\s*\n+/g, '\n\n').trim()
    : text.replace(/\s+/g, ' ').trim();
  if (!text || PAGE_TITLE_NOISE.test(text) || /^(?:\.?st\d+\s*\{[^}]*\}\s*)+$/i.test(text)) return null;
  return text;
}

function cleanImageUrls(value) {
  const candidates = urlArray(value) || [];
  const expanded = candidates.flatMap((candidate) => {
    const decoded = String(candidate || '').replace(/%3E/gi, '>');
    const embedded = [...decoded.matchAll(/https?:\/\/[^\s,>]+/gi)].map((match) => match[0]);
    // A normal absolute URL also matches; prefer the last host when a broken
    // relative wrapper contains another absolute image URL.
    return embedded.length > 1 ? embedded.slice(1) : embedded;
  });
  const selected = new Map();
  for (const candidate of expanded) {
    if (!/^https?:\/\//i.test(candidate)) continue;
    if (/\.(?:svg|ico)(?:[?#]|$)/i.test(candidate)) continue;
    if (/(?:^|[\/_-])(?:logos?|favicons?|sprites?|avatars?|icons?|markers?|loading|placeholders?|spacers?|blank)(?:[\/_\-.]|$)/i.test(candidate)) continue;
    if (/broadbandavailability|broadband(?:uk)?[-_]?badge/i.test(candidate)) continue;
    let key = candidate;
    let score = 0;
    try {
      const url = new URL(candidate);
      key = `${url.origin}${url.pathname}`.toLowerCase();
      const dimensions = decodeURIComponent(url.searchParams.get('size') || '').match(/(\d+)[,x](\d+)/i);
      const width = Number(url.searchParams.get('width')) || 0;
      const height = Number(url.searchParams.get('height')) || width;
      score = dimensions ? Number(dimensions[1]) * Number(dimensions[2]) : width ? width * height : (url.search ? 1 : 2);
    } catch { /* Keep the inspectable candidate as its own key. */ }
    const previous = selected.get(key);
    if (!previous || score > previous.score) selected.set(key, { url: candidate, score });
  }
  const cleaned = [...selected.values()].map((item) => item.url);
  return cleaned.length ? cleaned : null;
}

function sanitizePlatformData(platform) {
  for (const field of STRING_FIELDS) platform[field] = cleanPlatformText(platform[field], field);
  for (const field of ARRAY_FIELDS) {
    if (field === 'images' || field === 'floorPlan') platform[field] = cleanImageUrls(platform[field]);
    else if (Array.isArray(platform[field])) {
      const values = platform[field].map((value) => cleanPlatformText(value, field)).filter(Boolean);
      platform[field] = values.length ? [...new Set(values)] : null;
    }
  }

  // Gallery extractors sometimes include floorplans in the photo carousel.
  // Preserve them, but put them in the schema field intended for floorplans.
  if (Array.isArray(platform.images)) {
    const plans = platform.images.filter((url) => /(?:^|[\/_-])floor[\s_-]?plans?(?:[\/_\-.]|$)/i.test(url));
    if (plans.length) {
      platform.floorPlan = cleanImageUrls([...(platform.floorPlan || []), ...plans]);
      platform.images = platform.images.filter((url) => !plans.includes(url));
      if (!platform.images.length) platform.images = null;
    }
  }

  if (!platform.address && platform.location) platform.address = platform.location;
  if (!platform.location && platform.address) platform.location = platform.address;
  if (!platform.title || STATUS_ONLY_TITLE.test(platform.title)) {
    platform.title = platform.address || platform.location || null;
  }
  return platform;
}

function assessPlatformQuality(platform) {
  const missing = [];
  const present = (value) => value !== null && value !== undefined && value !== '' && (!Array.isArray(value) || value.length > 0);
  for (const field of ['title', 'purpose', 'price', 'currency', 'typeOfProperty', 'description']) {
    if (!present(platform[field]) || platform[field] === 'unknown') missing.push(field);
  }
  if (!present(platform.address) && !present(platform.location)) missing.push('address');
  if (!present(platform.images)) missing.push('images');
  if (!present(platform?._source?.url)) missing.push('sourceUrl');
  const qualityIssues = missing.map((field) => `Missing import-required field: ${field}`);
  return { missingImportFields: missing, qualityIssues, publishReady: missing.length === 0 };
}
...
function preserveAndRecoverPlatformData(platform, record, sourceUrl) {
  ...
  if (platform._normalization) {
    ...
    platform._normalization.missingFields = Object.entries(platform)
      ...
      .map(([field]) => field);
    const quality = assessPlatformQuality(platform);
    platform._normalization.criticalMissing = quality.missingImportFields;
    platform._normalization.qualityIssues = quality.qualityIssues;
    platform._normalization.imageCount = platform.images?.length || 0;
    platform._normalization.publishReady = quality.publishReady && platform._normalization.confidence >= 0.8
...
async function callOpenRouter(record, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      normalized._normalization.provider = isGemini ? 'gemini' : 'openrouter';
  ...
  async function worker() {
    while (nextIndex < records.length) {
      ...
      try {
        ...
        failed += 1;
        const configuredProvider = (options.provider || process.env.AI_PROVIDER
          || (options.apiKey || process.env.OPENROUTER_API_KEY ? 'openrouter' : 'gemini')).toLowerCase();
        const failedPlatform = preserveAndRecoverPlatformData({
            ...
            _normalization: {
              status: 'failed',
              provider: configuredProvider === 'gemini' ? 'gemini' : 'openrouter',
              model: options.model || (configuredProvider === 'gemini' ? process.env.GEMINI_MODEL : process.env.OPENROUTER_MODEL) || DEFAULT_MODEL,
              confidence: 0,
              requiresReview: true,
