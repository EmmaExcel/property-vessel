...
You receive 2-3 sample records from the same source website. Every record from this source shares the same field schema.
...
function pickSamples(records, maxSamples) {
  ...
  const indices = [0, Math.floor(records.length / 2), records.length - 1];
  const unique = [...new Set(indices)].slice(0, maxSamples);
...
async function loadOrGenerateRecipe(sourceUrl, records, options = {}) {
  ...
  const recipe = await generateMappingRecipe(records, options);
