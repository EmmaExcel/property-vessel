...
function extractFieldsFromCard($, $card) {
  ...
  if (link.length) {
    ...
    if (linkText && linkText.length < 200 && linkText.length > 3) {
      record.title = linkText;
    }
  }
  ...
  const addressPatterns = /\b\d+\s+[\w\s]+(?:road|street|lane|drive|avenue|close|way|crescent|terrace|place|court|grove)\b/i;
  const addressMatch = fullText.match(addressPatterns);
  if (addressMatch) record.address = addressMatch[0].trim();
