...
async function configureLightweightPage(page) {
  ...
  page.on('request', (request) => {
    ...
    const isTracker = /(?:google-analytics|googletagmanager|doubleclick|facebook\.com\/tr|hotjar|clarity\.ms|segment\.io)/i.test(requestUrl);
    try {
      if (isHeavyAsset || isTracker) request.abort();
      else request.continue();
    } catch {
      // Navigation can close while an intercepted request is being resolved.
    }
  });
  return page;
}
