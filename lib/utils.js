...
function mergeRecordValues(existing, incoming, key = '') {
  ...
  if (/title|name|headline|address|location/i.test(key)
      && typeof existing === 'string' && /^(?:load|show|view)\s+(?:previous|next|more|items|results)|\.st\d+\s*\{|fill\s*:|<svg|@keyframes/i.test(existing)
